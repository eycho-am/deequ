  ...
  "Verification Suite" should {
    ...
    "generate a result that contains row-level results" in withSparkSession { session =>
      ...
      val minLength = new Check(CheckLevel.Error, "rule4")
        .hasMinLength("item", _ <= 3, convertNull = Option(ConvertNulls(true)))
      val maxLength = new Check(CheckLevel.Error, "rule5")
        .hasMaxLength("item", _ > 1, convertNull = Option(ConvertNulls(true)))
      val expectedColumn1 = isComplete.description
    ...
    "generate a result that contains row-level results for length" in withSparkSession { session =>
      ...
      val isLengthMin = new Check(CheckLevel.Error, "rule3")
        .hasMinLength("att2", _ <= 1, convertNull = Option(ConvertNulls(true)))
      val isLengthMax = new Check(CheckLevel.Error, "rule4")
        .hasMaxLength("att2", _ >= 1, convertNull = Option(ConvertNulls(true)))
